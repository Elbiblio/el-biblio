...
class XPService {
  ...
  late final Box<dynamic> _xpBox;
  late final Box<int> _xpTotalBox;
  
  final StreamController<int> _xpStreamController = StreamController<int>.broadcast();
  Stream<int> get xpStream => _xpStreamController.stream;

  Future<void> initialize() async {
    _xpBox = await Hive.openBox<dynamic>(HiveBoxes.xpActivities);
    _xpTotalBox = await Hive.openBox<int>(HiveBoxes.xpTotals);
  }
  ...
  List<XPActivity> getTodayActivities() {
    ...
    return _xpBox.values.map((map) => XPActivity.fromMap(map as Map<String, dynamic>)).where((activity) {
      return activity.timestamp.isAfter(today) && activity.timestamp.isBefore(tomorrow);
    }).toList();
  }
  ...
  List<XPActivity> getActivitiesInRange(DateTime start, DateTime end) {
    return _xpBox.values.map((map) => XPActivity.fromMap(map as Map<String, dynamic>)).where((activity) {
      return activity.timestamp.isAfter(start) && activity.timestamp.isBefore(end);
    }).toList();
  }

  /// Get XP earned today
  int getTodayXP() {
    return getTodayActivities().fold(0, (sum, activity) => sum + activity.xpAmount);
  }
  ...
  int getWeeklyXP() {
    ...
    return getActivitiesInRange(weekStartDay, now).fold(0, (sum, activity) => sum + activity.xpAmount);
  }
  ...
  int getMonthlyXP() {
    ...
    return getActivitiesInRange(monthStart, now).fold(0, (sum, activity) => sum + activity.xpAmount);
  }
  ...
  Map<XPActivityType, int> getXPBreakdown() {
    ...
    for (final map in _xpBox.values) {
      ...
      breakdown[activity.type] = (breakdown[activity.type] ?? 0) + activity.xpAmount;
    }
