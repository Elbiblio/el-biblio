...
class BibleRepository extends BaseRepository {
    ...
    return guard(() async {
      ...
      try {
        ...
        try {
          ...
          logger.w(
            'Failed to get books from local database, falling back to API: $e',
          );
        }
  ...
  Future<List<BibleVerseContent>> getVerses(
    String version,
    String book,
    int chapter, {
    ...
    return guard(() async {
      logger.d(
        'Getting verses for version: $version, book: $book, chapter: $chapter',
      );
      ...
      try {
        final response = await _client.get(
          '/bible/verses/$version/$book/$chapter',
        );
        ...
        for (final item in data) {
          try {
            if (item is Map<String, dynamic>) {
              ...
              final verse = _mapApiVerseToContent(
                item,
                book,
                chapter,
                version: version,
        ...
        if (version == 'eng_rv_vpl' ||
            version == 'RSV' ||
            version == 'engdra_vpl') {
          logger.d('API failed, trying bundled database as fallback');
          try {
            final verses = await _dbService.getChapter(
              'eng_rv_vpl',
              book,
              chapter,
            );
  ...
  BibleVerseContent? _mapApiVerseToContent(
    ...
    try {
      ...
      final id = json['id'] != null
          ...
          : int.parse(
                  sha256
                      .convert(
                        utf8.encode(
                          '${version ?? 'api'}:$book $chapter:$verse',
                        ),
                      )
                      .toString()
                      .substring(0, 8),
      ...
      final reference =
          json['reference']?.toString() ?? '$book $chapter:$verse';

      if (verse <= 0 || text.isEmpty) {
        logger.w(
          'Invalid verse data: verse=$verse, text="${text.length > 50 ? text.substring(0, 50) : text}..."',
        );
        return null;
      }
  ...
  Future<List<BibleVersion>> getVersions() {
    return guard(() async {
      ...
      try {
        ...
        if (response.data is List) {
          ...
          for (final item in response.data) {
            try {
              ...
              logger.e(
                'Error parsing BibleVersion from versions API: $e, data: $item',
              );
            }
          }

          // Check download status for each
          final checkedVersions = await Future.wait(
            versions.map((v) async {
              if (v.tableName != null) {
                final downloaded = await _dbService.isVersionDownloaded(
                  v.tableName!,
                );
        ...
        logger.e(
          'Failed to fetch from versions API, falling back to regular API: $e',
        );
      }
        ...
        try {
          if (item is Map<String, dynamic>) {
            ...
            if (item['abbreviation'] != null) {
              ...
              logger.w(
                'Skipping BibleVersion with missing required fields: $item',
              );
            }
          }
        } catch (e) {
  ...
  Future<void> downloadVersionWithUrl(
    String versionId, {
    required String downloadUrl,
    required Function(int, int) onProgress,
  }) {
    return guard(() async {
      await _dbService.downloadVersionWithUrl(
        versionId,
    ...
    return guard(
      () async {
        ...
        if (isGuestToken(token)) {
          logger.i(
            'Guest user detected, returning offline insight for verse $verseId',
          );
          return const BibleInsight(
            ...
            sections: [
              InsightSection(
                title: 'Guest Mode',
                content: 'Create an account to open AI-powered insights.',
              ),
            ],
          );
        }
        ...
        if (data is Map<String, dynamic>) {
          if (data['sections'] != null) {
            ...
            sections = [
              InsightSection(title: 'Insight', content: data['content']),
            ];
          }
  ...
  Future<List<BibleVerseContent>> searchVerses(String version, String query) {
    return guard(() async {
      ...
      final response = await _client.get(
        '/bible/search',
        queryParameters: {'q': query, 'version': version},
      );
      ...
      for (final item in data) {
        try {
          if (item is Map<String, dynamic>) {
            ...
            final book =
                item['book']?.toString() ?? item['book_abbr']?.toString() ?? '';
            final chapter =
                int.tryParse(item['chapter']?.toString() ?? '0') ?? 1;
  ...
  Future<List<Map<String, dynamic>>> compareVerses(
    String version,
    String reference,
  ) {
    return guard(() async {
  ...
  Future<void> saveVerseNote(int verseId, String note) {
    return guard(() async {
      ...
      await _client.post(
        '/notes',
        data: {'verse_id': verseId, 'content': note, 'type': 'verse_note'},
      );
  ...
  Future<List<Map<String, dynamic>>> getVerseNotes(int verseId) {
    return guard(() async {
      final response = await _client.get(
        '/notes',
        queryParameters: {'verse_id': verseId, 'type': 'verse_note'},
      );
      ...
      for (int i = 0; i < count && i < randomChapters.length; i++) {
        ...
        try {
          ...
          if (verses.isNotEmpty) {
            ...
            final apiVerses = await getVerses(
              'eng_rv_vpl',
              chapter.book,
              chapter.chapter,
            );
          ...
          logger.w(
            'Failed to get verses for ${chapter.book} ${chapter.chapter}: $e',
          );
  ...
  List<_RandomChapter> _getInspirationalChapters() {
    return [
      // Wisdom Literature
      const _RandomChapter('PRO', 1),
      const _RandomChapter('PRO', 3),
      const _RandomChapter('PRO', 4),
      const _RandomChapter('PRO', 15),
      const _RandomChapter('PRO', 16),
      const _RandomChapter('PRO', 31),
      const _RandomChapter('ECC', 3), const _RandomChapter('ECC', 12),
      const _RandomChapter('JOB', 1),
      const _RandomChapter('JOB', 38),
      const _RandomChapter('JOB', 42),

      // Psalms (key inspirational psalms)
      const _RandomChapter('PSA', 1),
      const _RandomChapter('PSA', 23),
      const _RandomChapter('PSA', 27),
      const _RandomChapter('PSA', 34),
      const _RandomChapter('PSA', 46),
      const _RandomChapter('PSA', 51),
      const _RandomChapter('PSA', 91),
      const _RandomChapter('PSA', 103),
      const _RandomChapter('PSA', 119),
      const _RandomChapter('PSA', 121),
      const _RandomChapter('PSA', 139),
      const _RandomChapter('PSA', 145),
      const _RandomChapter('PSA', 150),

      // Gospels
      const _RandomChapter('MAT', 5),
      const _RandomChapter('MAT', 6),
      const _RandomChapter('MAT', 7),
      const _RandomChapter('MAT', 11),
      const _RandomChapter('MAT', 13),
      const _RandomChapter('MAT', 25),
      const _RandomChapter('MAR', 4),
      const _RandomChapter('MAR', 5),
      const _RandomChapter('MAR', 12),
      const _RandomChapter('LUK', 6),
      const _RandomChapter('LUK', 10),
      const _RandomChapter('LUK', 12),
      const _RandomChapter('LUK', 15),
      const _RandomChapter('LUK', 18),
      const _RandomChapter('JOH', 1),
      const _RandomChapter('JOH', 3),
      const _RandomChapter('JOH', 6),
      const _RandomChapter('JOH', 10),
      const _RandomChapter('JOH', 14),
      const _RandomChapter('JOH', 15),

      // Paul's Letters
      const _RandomChapter('ROM', 3),
      const _RandomChapter('ROM', 5),
      const _RandomChapter('ROM', 8),
      const _RandomChapter('ROM', 12),
      const _RandomChapter('ROM', 15),
      const _RandomChapter('1CO', 13), const _RandomChapter('1CO', 15),
      const _RandomChapter('2CO', 4),
      const _RandomChapter('2CO', 5),
      const _RandomChapter('2CO', 12),
      const _RandomChapter('GAL', 5), const _RandomChapter('GAL', 6),
      const _RandomChapter('EPH', 1),
      const _RandomChapter('EPH', 2),
      const _RandomChapter('EPH', 4),
      const _RandomChapter('EPH', 6),
      ...
      const _RandomChapter('2TI', 1),
      const _RandomChapter('2TI', 2),
      const _RandomChapter('2TI', 3),

      // Hebrews
      const _RandomChapter('HEB', 1),
      const _RandomChapter('HEB', 4),
      const _RandomChapter('HEB', 11),
      const _RandomChapter('HEB', 12),
      const _RandomChapter('HEB', 13),

      // General Letters
      const _RandomChapter('JAS', 1),
      const _RandomChapter('JAS', 2),
      const _RandomChapter('JAS', 3),
      const _RandomChapter('JAS', 4),
      const _RandomChapter('JAS', 5),
      const _RandomChapter('1PE', 1),
      const _RandomChapter('1PE', 2),
      const _RandomChapter('1PE', 3),
      const _RandomChapter('1PE', 4),
      const _RandomChapter('1PE', 5),
      const _RandomChapter('1JO', 1),
      const _RandomChapter('1JO', 2),
      const _RandomChapter('1JO', 3),
      const _RandomChapter('1JO', 4),

      // Old Testament Inspirational
      const _RandomChapter('ISA', 1),
      const _RandomChapter('ISA', 6),
      const _RandomChapter('ISA', 11),
      const _RandomChapter('ISA', 26),
      const _RandomChapter('ISA', 40),
      const _RandomChapter('ISA', 41),
      const _RandomChapter('ISA', 53),
      const _RandomChapter('ISA', 55),
      const _RandomChapter('JER', 17),
      const _RandomChapter('JER', 29),
      const _RandomChapter('JER', 31),
      const _RandomChapter('JER', 33),
      const _RandomChapter('EZE', 18),
      const _RandomChapter('EZE', 33),
      const _RandomChapter('EZE', 36),
      const _RandomChapter('EZE', 37),
      const _RandomChapter('DAN', 2),
      const _RandomChapter('DAN', 3),
      const _RandomChapter('DAN', 6),
      const _RandomChapter('DAN', 12),
      const _RandomChapter('JOS', 1), const _RandomChapter('JOS', 24),
      const _RandomChapter('GEN', 1),
      const _RandomChapter('GEN', 2),
      const _RandomChapter('GEN', 12),
      const _RandomChapter('GEN', 15),
      const _RandomChapter('GEN', 22),
      const _RandomChapter('GEN', 28),
      const _RandomChapter('GEN', 37),
      const _RandomChapter('GEN', 45),
      const _RandomChapter('GEN', 50),
      const _RandomChapter('EXO', 3),
      const _RandomChapter('EXO', 14),
      const _RandomChapter('EXO', 19),
      const _RandomChapter('EXO', 20),
      const _RandomChapter('EXO', 33),
      const _RandomChapter('EXO', 34),
      const _RandomChapter('DEU', 6),
      const _RandomChapter('DEU', 30),
      const _RandomChapter('DEU', 31),
    ];
  }
