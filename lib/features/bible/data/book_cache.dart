...
class BookCache {
  ...
  static List<BibleBookDefinition> getOldTestamentBooks() {
    _oldTestamentBooks ??= standardBibleBooks.where((book) => book.testament == 'OT').toList();
  ...
  static List<BibleBookDefinition> getNewTestamentBooks() {
    _newTestamentBooks ??= standardBibleBooks.where((book) => book.testament == 'NT').toList();
  ...
  static Map<String, BibleBookDefinition> getBookMap() {
    _bookMap ??= Map.fromEntries(
      standardBibleBooks.map((book) => MapEntry(book.abbreviation, book))
    );
    return _bookMap!;
  }
