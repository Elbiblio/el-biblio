...
Map<String, dynamic> extractReadingInfo(Activity activity) {
  ...
  if ((book == null || book.isEmpty) && metadata?['chapters_read'] != null) {
    ...
    final chaptersRead = rawChaptersRead is List
        ? rawChaptersRead
        : <dynamic>[rawChaptersRead];
  ...
  return {'book': book ?? 'Genesis', 'chapter': chapter ?? 1, 'verse': verse};
}
...
String getMotivationalGreeting(BibleReadingState readingState) {
  if (readingState.history.isEmpty) return 'Bible';
  ...
  if (streak >= 3) return 'Keep reading';
  if (totalDays >= 1) return 'Continue';
  return 'Bible';
}
...
String getMotivationalSubtitle(BibleReadingState readingState) {
  ...
  if (streak >= 30) return '$streak days in Scripture.';
  if (streak >= 14) return '$streak days in Scripture.';
  if (streak >= 7) return '$streak days in Scripture.';
  if (streak >= 3) return '$streak days in Scripture.';
  if (streak == 1) return 'Read again today.';
  if (lastRead != null) {
...
List<Color> getGradientColors(String? themeId) {
  switch (themeId) {
    ...
    case 'love':
      return [Colors.red.shade400, Colors.pink.shade700];
    case 'knowledge':
    ...
    case 'hope':
      return [Colors.lightBlue.shade400, Colors.blue.shade600];
    case 'joy':
      return [Colors.yellow.shade400, Colors.amber.shade700];
    case 'peace':
    ...
    case 'goodness':
      return [Colors.lime.shade400, Colors.green.shade600];
    case 'faithfulness':
      return [Colors.blue.shade500, Colors.blue.shade800];
    case 'gentleness':
      return [Colors.purple.shade300, Colors.purple.shade600];
    case 'self_control':
      return [Colors.grey.shade400, Colors.grey.shade700];
    default:
      return [Colors.teal.shade400, Colors.cyan.shade700];
  }
}
...
Map<String, dynamic> parseReference(String reference) {
  final match = RegExp(r'^\s*(.+?)\s+(\d+)(?::(\d+))?').firstMatch(reference);
  if (match == null) {
