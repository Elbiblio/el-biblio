...
class FontSizeCalculator {
  static double calculateOptimalFontSize({
    ...
    // Adjust based on word count - more words = smaller font
    if (wordCount > 15) {
    ...
    // Further adjust for very long words
...
class _VerseGameScreenState extends ConsumerState<VerseGameScreen> with SingleTickerProviderStateMixin {
  ...
  void _generateRandomTransformations() {
    _rotations = List.generate(50, (index) => (_random.nextDouble() * 10 - 5) * pi / 180);
    _translations = List.generate(50, (index) => Offset(_random.nextDouble() * 20 - 10, _random.nextDouble() * 20 - 10));
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Listen for completion to play sound
    ref.listen(verseGameProvider.select((state) => state.state), (previous, next) {
      if ((next == GameState.success || next == GameState.sessionComplete) && 
          previous != GameState.success && previous != GameState.sessionComplete) {
        ref.read(soundServiceProvider).playGameSuccess();
        HapticFeedback.heavyImpact();
      } else if (next == GameState.failed && previous != GameState.failed) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: gameState.state == GameState.loading
            ...
            : gameState.state == GameState.gameOver || gameState.state == GameState.sessionComplete
                ? _buildEndScreen(context, gameState, notifier, primaryColor, textColor)
                : Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            // Top row with close, progress, and lives
                            Row(
                              ...
                              children: [
                                ...
                                // Progress Counter with difficulty indicator
                                Column(
                                    ...
                                    if (gameState.difficulty != null) ...[
                                      Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: _getDifficultyColor(gameState.difficulty!.category),
                                          borderRadius: BorderRadius.circular(12),
                                        ),
                                        child: Text(
                                          gameState.difficulty!.category.name.toUpperCase(),
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: List.generate(3, (index) {
                                        return Icon(
                                          index < gameState.lives ? Icons.favorite : Icons.favorite_border,
                                          color: index < gameState.lives ? Colors.red : mutedTextColor,
                                          size: 16,
                                        );
                                      }),
                                    ),
                      ...
                      Text(
                        gameState.currentMode == GameMode.arrange ? 'ARRANGE THE VERSE' : 'GUESS THE WORD',
                        style: TextStyle(
                          fontSize: 10,
                          fontWeight: FontWeight.bold,
                      ...
                      if (gameState.currentMode == GameMode.guess)
                        _buildGuessContext(gameState, textColor, primaryColor)
                      else if (gameState.currentMode == GameMode.arrange)
                        _buildArrangeContext(gameState, textColor, primaryColor, mutedTextColor),
                      ...
                      // Footer Messages
                      if (gameState.state == GameState.success || gameState.state == GameState.checking)
                        Container(
                          padding: const EdgeInsets.all(16),
                          margin: const EdgeInsets.only(bottom: 24, left: 16, right: 16),
                          decoration: BoxDecoration(
                            color: gameState.state == GameState.success ? Colors.green.withValues(alpha: 0.1) : Colors.blue.withValues(alpha: 0.1),
                            borderRadius: BorderRadius.circular(16),
                          ),
                          child: Column(
                            children: [
                              Text(
                                gameState.state == GameState.success ? 'Great job! Loading next question...' : 'Checking...',
                                style: TextStyle(
                                  color: gameState.state == GameState.success ? Colors.green : Colors.blue, 
                                  fontWeight: FontWeight.bold
                                ),
                              ),
                              if (gameState.state == GameState.success && gameState.currentMode == GameMode.arrange) ...[
                                const SizedBox(height: 8),
                                Text(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    gameState.currentMode == GameMode.arrange ? 'TAP IN ORDER' : 'TAP THE ANSWER',
                                    style: TextStyle(
                                      fontSize: 11,
                                      fontWeight: FontWeight.bold,
                                      color: mutedTextColor,
  ...
  Widget _buildGuessContext(VerseGameState gameState, Color textColor, Color primaryColor) {
              ...
              return Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    ...
                    height: 1.2, // Reduced from 1.5 to prevent clipping
                  ),
                ),
              );
            }).toList(),
  ...
  Widget _buildArrangeContext(VerseGameState gameState, Color textColor, Color primaryColor, Color mutedColor) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final optimalFontSize = FontSizeCalculator.calculateContextFontSize(
          words: gameState.selectedIndices.map((index) => gameState.shuffledWords[index]).toList(),
          availableWidth: constraints.maxWidth,
      ...
      builder: (context, constraints) {
        ...
        final wrap = Wrap(
          ...
          children: words.asMap().entries.map((entry) {
            ...
            final isSelected = gameState.currentMode == GameMode.arrange &&
            ...
            final chip = AnimatedScale(
              ...
              child: Transform.translate(
                offset: gameState.currentMode == GameMode.arrange ? _translations[transformIndex] : Offset.zero,
                child: Transform.rotate(
                  angle: gameState.currentMode == GameMode.arrange ? _rotations[transformIndex] : 0,
                  child: Material(
        ...
        if (shouldScroll) {
          return SingleChildScrollView(
            ...
            child: ConstrainedBox(
              constraints: BoxConstraints(minHeight: constraints.maxHeight * 0.9),
  ...
  Widget _buildEndScreen(BuildContext context, VerseGameState gameState, VerseGameNotifier notifier, Color primaryColor, Color textColor) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: Text('Back to Bible', style: TextStyle(color: textColor.withValues(alpha: 0.6))),
          ),
        ],
      ),
    );
